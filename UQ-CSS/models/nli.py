...
class ClassifyWrapper():
    ...
    def __init__(self, model_name='openai/all-MiniLM-L6-v2', device='cuda') -> None:
    ...
    def _batch_pred(self, sen_1: list, sen_2: list, max_batch_size=128):
        ...
        for st in range(0, len(input_ids), max_batch_size):
            ...
            temp = self.model(input_ids=input_ids[st:ed],
                                attention_mask=attention_mask[st:ed])
#             print('temp', temp.size())
            logits.append(temp)
#         print('logits', torch.cat(logits, dim=0).size())
...
#         return torch.cat(logits, dim=0)
    ...
    def create_sim_mat_batched(self, question, answers):
        ...
        sim_mat_batch = torch.zeros((len(unique_ans), len(unique_ans),512))
...
#             print('sim_mat_batch_flat', sim_mat_batch_flat.size())
