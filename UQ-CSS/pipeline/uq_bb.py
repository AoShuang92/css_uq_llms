...
def _compute_lexical_sim(sample, num_gens=20):
...
def recover_sim_mat_new(sim, n_components):
#     print('n_components', n_components, type(n_components))
    
    sim_mat = sim['sim_mat'].clone() 
    # sim_mat[torch.arange(sim_mat.shape[0]), torch.arange(sim_mat.shape[0]), :] = torch.tensor([-torch.inf, -torch.inf, 100])
    mapping = sim['mapping']
    # a len(ans) x len(ans) x 3 tensor
    ret = torch.zeros((len(mapping), len(mapping), sim_mat.shape[2]))
#     print('ret', ret.size())
    inf_indices = []
    for i, ans_i in enumerate(mapping):
        for j, ans_j in enumerate(mapping):
    ...
    logits = ret
    logits_ = logits.reshape(400,512).transpose(1,0)
    pca = PCA(n_components = n_components)
    fit = pca.fit(logits_)
    logits_reduced = fit.components_.transpose(1,0)
    
    logits_reduced_ = torch.tensor(logits_reduced).reshape(20,20,n_components)
    new_ret = logits_reduced_
#     new_ret = ret
    temp_mat = torch.ones(new_ret.shape[2])*100
    temp_mat[:-1] = torch.ones(new_ret.shape[2]-1)*-torch.inf

    for ids in inf_indices:
        new_ret[ids[0], ids[1], :] = temp_mat
...
class UQ_computer:
    def __init__(self, path, clean=True,
                 split=None, cal_size=None, seed=None, n_components=None, gpt_thr = None, rougeL_thr=None) -> None:
        ...
        if split is not None:
            ...
            self.key = (_clean_path(path) if self.path is not None else None, clean, split, cal_size, seed, n_components, gpt_thr, rougeL_thr )
        ...
        self.gpt_thr = gpt_thr
        self.rougeL_thr = rougeL_thr
    ...
    def _get_recovered_logits(self, num_gens:int):
        # print('recover_sim_mat_new', self.n_components)
        if '_get_recovered_logits' not in self.mem:
            self.mem['_get_recovered_logits'] = [recover_sim_mat_new(_, self.n_components)[1] for _ in self.similarities]
...
class UQ_summ(UQ_computer):
    ...
    default_params = {'eigv_threshold': 0.9, 'temperature': 1.5}
...
#         self.gpteval_examples = gpteval_examples
    ...
    def __init__(self, path, eigv_threshold=None, temperature = None, gpt_thr = None, rougeL_thr = None,  clean=True, 
                 split=None, cal_size=None, seed=None, n_components = None, 
                 gpteval_examples = None) -> None:
        
        super().__init__(path, clean, split, cal_size, seed, n_components, gpt_thr, rougeL_thr)
        self.gpteval_examples = gpteval_examples
    ...
    def get_acc(self, acc_name='generations|rougeL|acc'):
        ...
        if suffix == 'acc':
            if name == 'rougeL':
                indiv_acc = (indiv_acc > self.rougeL_thr).astype(float)
            elif name == 'gpt':
                # print('thr', self.rougeL_thr)
                indiv_acc = (indiv_acc > self.gpt_thr).astype(float)
        # print('mean_acc', indiv_acc.mean(1))
    ...
    def summ(self, uq_names, acc_name:str, num_gens=20, uq_kwargs:dict=None, overall=False, use_conf=True):
        if uq_kwargs is None:
            ...
            if len(self.key) > 2:
                ...
                self2 = self.__class__(self.path,clean=self.key[1], split='val', cal_size=self.key[3], seed=self.key[4], n_components=self.key[5], gpt_thr=self.key[6], rougeL_thr=self.key[7] )
